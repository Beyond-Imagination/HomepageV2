...
function getRetryDelayMs(status: number, retryAfterHeader: string | null) {
  if (status === 429) {
    ...
    console.warn(
      '[apply-notion-team-links] 429 received without a valid Retry-After header. Falling back to default backoff.'
    )
...
async function notionUpdatePage(pageId: string, payload: Record<string, unknown>) {
  for (let attempt = 1; attempt <= NOTION_API_MAX_RETRIES; attempt += 1) {
    try {
      ...
      console.warn(
        `[apply-notion-team-links] Notion update retry ${attempt}/${NOTION_API_MAX_RETRIES} after ${retryAfterMs}ms: ${response.status}`
      )
      ...
      console.warn(
        `[apply-notion-team-links] Notion update network retry ${attempt}/${NOTION_API_MAX_RETRIES} after ${NOTION_API_RETRY_DELAY_MS}ms`
      )
...
async function run() {
  ...
  if (updates.length === 0) {
    console.log('[apply-notion-team-links] No pending updates. Skipping.')
    return
  }

  console.log(`[apply-notion-team-links] Updating ${updates.length} Notion page link(s)...`)
  ...
  console.log(`[apply-notion-team-links] Updated ${updates.length} Notion page link(s).`)
}

run().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error)
  console.error('[apply-notion-team-links] Failed:', error)
  console.error('[apply-notion-team-links] Message:', message)
  process.exit(1)
})
